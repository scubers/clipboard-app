...
func ensureSchema(db *sql.DB) error {
	...
	if cur < 3 {
		...
		cur = 3
		if _, err := db.Exec(`INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version', '3')`); err != nil {
			return fmt.Errorf("set schema_version v3: %w", err)
		}
	}
	...
	if cur < 6 {
		...
		cur = 6
		if _, err := db.Exec(`INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version', '6')`); err != nil {
			return fmt.Errorf("set schema_version v6: %w", err)
		}
	}
